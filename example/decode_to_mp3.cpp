...
int main(int argc, char **argv)
{
	...
	if(argc > 1) 
	{
		...
		sprintf(out_filename, "%s.mp3", argv[1]);


		// set wave output to disk file with mp3 encoding
 		if(player->SetWaveOutFile(out_filename, sfMp3, 0) == 0)
		{
        	printf("Error: %s\nPress key to exit.\r\n", player->GetError());
            getch();
            player->Release();
