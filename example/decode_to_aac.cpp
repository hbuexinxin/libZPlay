/*
 *	Decode stream into AAC ADTS file.
 *
*/
...
int main(int argc, char **argv)
{
	...
	if(argc > 1) 
	{
		...
		sprintf(out_filename, "%s.aac", argv[1]);


		// set wave output to disk file with AAC encoding
 		if(player->SetWaveOutFile(out_filename, sfAacADTS, 0) == 0)
		{
        	printf("Error: %s\nPress key to exit.\r\n", player->GetError());
            getch();
            player->Release();
